...
class CalendarHelper extends Helper
{
	...
	*/
	function calendar($currentMonthsYear, $monthName, $data = NULL, $baseUrl = NULL){
		...
		$str .= $this->Html->link(__('prev', true), $baseUrl.'/' . $previousString);
		$str .= '</th><th colspan="5">' . ucfirst($monthName) . ' ' . $currentMonthsYear . '</th><th class="cell-next">';
		$str .= $this->Html->link(__('next', true), $baseUrl.'/' . $nextString);
		$str .= '</th></tr>';
		$str .= '<tr>';
		for($i = 0; $i < 7;$i++) {
			$str .= '<th class="cell-header">' . $this->day_list[$i] . '';
		}
		$str .= '</tr>';
		$str .= '</thead>';
		$str .= '<tbody>';
	...
	*/
	function week($year = '', $month = '', $day = '', $data = '', $base_url ='', $min_hour = 8, $max_hour=24)
	{
		...
		$str .= '<thead>';
		$str .= '<tr><th class="cell-prev">';
		$str .= $this->Html->link(__('prev', true), $base_url.'/' . $prev_year . '/' . $prev_month.'/'.$prev_week.$this->getParams());
		$str .= '</th><th colspan="5">' . $title . '</th><th class="cell-next">';
		$str .= $this->Html->link(__('next', true), $base_url.'/' . $next_year . '/' . $next_month.'/'.$next_week.$this->getParams());
		$str .= '</th></tr>';
		$str .= '<tr>';
		for($i = 0; $i < 7;$i++) {
			$offset = 0;
