...
class FileUploadHelper extends AppHelper{
  ...
  function _getImageByName(){
    ...
    if(!file_exists($this->_getFullPath())) return false;
  
    //resize if we have resize on, a width, and if it doesn't already exist.
    if($this->autoResize && $this->options['width'] && !file_exists($this->_getResizeNameOrPath($this->_getFullPath()))){
       $this->_resizeImage();
    }
  ...
  function _getResizeNameOrPath($file_name_or_path){
    $temp_path = substr($file_name_or_path, 0, strlen($file_name_or_path) - strlen($this->_getExt())); //temp path without the ext
    return $temp_path . "x" . $this->options['width'] . $this->_getExt();
  }
  ...
  function _resizeImage(){
    $new_image = new ResizeImage($this->_getFullPath());
  ...
  /**************************************************
    ...
    */
  function _htmlImage(){
    $image = $this->autoResize ? $this->_getResizeNameOrPath($this->_getImagePath()) : $this->_getImagePath();
    return $this->Html->image($image, $this->options);
  }
}
